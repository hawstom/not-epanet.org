...
| 2.11 | Curve editor modelled on EPANET's; table of assets is the view EPANET has had since 2.0; default colour scheme is EPANET 2.2's five map colours kept unsoftened | `EC` commit `7b146c1d` "Curve editor reads like EPANET's"; `EC/js/looped-network.js` comment on the tables pane, "EPANET has had it since 2.0"; `EC/js/lpn-ramps.js`: "The five map colours of EPANET 2.2 ... Kept unsoftened ... It is our heritage default" |
...
| 3.7 | No foundation, no board, no governance document, no funding | `EC/dev/positioning.md` §1 and §8: "There is no foundation, no board, no governance document and no funding model" |
...
| 4.13 | All third-party code is vendored; a copy travels with the software rather than being fetched while you work | `EC/js/vendor/README.md`: "Everything this site loads comes from this site. There is no CDN, no hosted font, and no third-party code of any kind." Map tiles are the stated data exception, and the site says the tiles come from those services |
